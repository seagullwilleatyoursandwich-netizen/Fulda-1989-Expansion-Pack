...
namespace Fulda1989
{
    public class PactThermal : Module
    {
        ...
        {
            AssetBundle thermal_bundle = AssetBundle.LoadFromFile(Path.Combine(MelonEnvironment.ModsDirectory + "/Fulda1989", "pact_thermals")
            );

            if (thermal_bundle == null)
            {
                // MelonLogger.Error("PACT THERMALS BUNDLE FAILED TO LOAD");
                return;
            }

            // MelonLogger.Msg("PACT THERMALS BUNDLE LOADED");

            // foreach (var asset in thermal_bundle.GetAllAssetNames())
            //      MelonLogger.Msg(asset);

            colour_ramp_white = thermal_bundle.LoadAsset<Texture>("Fulda FLIR White");

            if (colour_ramp_white == null)
                MelonLogger.Error("FAILED TO LOAD FLIR WHITE");
            else
                MelonLogger.Msg("FLIR WHITE LOADED");
        }
    }
}
