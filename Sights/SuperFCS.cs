...
namespace Fulda1989
{
    public class SuperFCS : Module
    {
        ...
        static GameObject monitor_sosna;
        static GameObject monitor_vesna;
            ...
            WeaponSystemInfo main, CustomGuidanceComputer mgu, bool vesna = false) 
        {
            ...
            */
            ...
            GameObject monitor_canvas = GameObject.Instantiate(vesna ? monitor_vesna : monitor_sosna, night_optic.transform);
        ...
        {
            AssetBundle sosna_bundle = AssetBundle.LoadFromFile(Path.Combine(MelonEnvironment.ModsDirectory + "/Fulda1989", "monitor_sosna"));

            if (sosna_bundle == null)
            {
                MelonLogger.Error("FULDA SOSNA BUNDLE FAILED TO LOAD");
            }
            else
            {
                MelonLogger.Msg("FULDA SOSNA BUNDLE LOADED");

                // foreach (var asset in sosna_bundle.GetAllAssetNames())
                //    MelonLogger.Msg(asset);
            }

            monitor_sosna = sosna_bundle.LoadAsset<GameObject>("fulda sosna monitor canvas");
            monitor_sosna.hideFlags = HideFlags.DontUnloadUnusedAsset;

            AssetBundle vesna_bundle = AssetBundle.LoadFromFile(Path.Combine(MelonEnvironment.ModsDirectory + "/Fulda1989", "monitor_vesna"));
            monitor_vesna = vesna_bundle.LoadAsset<GameObject>("fulda vesna k canvas");
            monitor_vesna.hideFlags = HideFlags.DontUnloadUnusedAsset;
        }
    }
}
